// creditCardRevolving.ts
// ยอดใช้จ่ายหมุนเวียน (section 5) · สรุปข้อมูล (section 6) · validation (section 7)
//
// ย้ายออกมาจาก creditCardUtils.js (TD-H02 sub-slice 3/3, FINAL) — creditCardUtils.js ยังคง
// re-export สัญลักษณ์ทั้ง 13 ตัวที่ export จากไฟล์นี้ ไม่มีการเปลี่ยนพฤติกรรมใด ๆ ในไฟล์นี้ —
// เฉพาะการเพิ่ม type annotation เท่านั้น (ยกเว้นจุดที่ TypeScript's strict mode บังคับให้เปลี่ยน
// .includes() บน readonly const-tuple เป็น .some() เพื่อเลี่ยงการเพิ่ม `as` cast อีกจุด — ผลลัพธ์
// รันไทม์เหมือนเดิมทุกกรณี ดูรายละเอียดใน changes-td-h02-creditcard-revolving.md)
...
// actually reads/returns, not full domain types (same precedent as
// creditCardAmortization.ts's ScheduleRow/ScheduleSummary)
// ---------------------------------------------------------------------------

/** การตัดสินใจชำระของยอดหมุนเวียน — null = ยังไม่ตัดสินใจ (BR-CC-014) */
type PaymentAction = 'full' | 'minimum' | null;

/** บัตร — เฉพาะฟิลด์ที่ฟังก์ชันในไฟล์นี้อ่านจริง */
type CardLike = {
  id?: string;
...
/** cycle ยอดหมุนเวียนที่เก็บจริง (input ของ buildRevolvingCycles) */
type StoredRevolvingCycle = {
  id?: string;
  cardId?: string;
  month?: string;
...
/** cycle ที่ derive แล้ว — ผลลัพธ์ของ buildRevolvingCycles ใช้ต่อโดย summariseRevolving/resolveCardNextDueDate/summariseCard */
type RevolvingCycleRow = {
  id: string | null;
  cardId: string;
  month: string;
  newSpend: number;
...
/** ผลสรุปยอดหมุนเวียน ณ เดือนหนึ่ง (ผลลัพธ์ของ summariseRevolving) */
type RevolvingSummary = {
  totalDue: number;
  minDue: number;
  due: number;
  outstanding: number;
  carryForward: number;
  paymentAction: PaymentAction;
  cycle: RevolvingCycleRow | null;
};

/** แถวเดียวในตารางผ่อน — เฉพาะฟิลด์ที่ section 6 อ่านจริง */
type PlanScheduleRowLike = {
  no?: number;
  dueMonth?: string;
...
/** แผนผ่อน — เฉพาะฟิลด์ที่ section 6 อ่านจริง */
type PlanLike = {
  id?: string;
  cardId?: string;
  status?: string;
  schedule?: PlanScheduleRowLike[];
  [key: string]: unknown;
};

/** ค่าที่เป็นไปได้ของสถานะแผน */
type PlanStatusValue = (typeof PLAN_STATUS)[keyof typeof PLAN_STATUS];
...
function clampPercent(value: number | string, min: number, max: number, fallback: number): number {
 ...
 * @returns {object}
 */
export function normaliseCardDefaults(card: CardLike = {}): CardLike & { annualRate: number; minPaymentPercent: number } {
  return {
    ...card,
...
export function isValidPaymentAction(value: unknown): value is PaymentAction {
  if (value === null) return true;
  return typeof value === 'string' && PAYMENT_ACTIONS.some(action => action === value);
}
 ...
 */
function withTruncationFlag<T extends unknown[]>(cycles: T, truncated: boolean): T & { truncated: boolean } {
  // Object.defineProperty's own type signature returns the original object type (T), not the
  // intersection it actually produces at runtime — TypeScript cannot narrow this on its own,
  // so this one documented cast expresses the shape the call already guarantees at runtime.
  return Object.defineProperty(cycles, 'truncated', {
 ...
 */
export function buildRevolvingCycles(
  card: CardLike = {},
  storedCycles: StoredRevolvingCycle[] = [],
  options: { throughMonth?: string } = {}
): RevolvingCycleRow[] & { truncated: boolean } {
  const cardId = card?.id;
  if (!cardId) return withTruncationFlag<RevolvingCycleRow[]>([], false);

  const cycles = (Array.isArray(storedCycles) ? storedCycles : [])
    .filter((cycle): cycle is StoredRevolvingCycle & { month: string } =>
      cycle?.cardId === cardId && typeof cycle?.month === 'string' && MONTH_KEY_RE.test(cycle?.month ?? ''))
    .sort((a, b) => a.month.localeCompare(b.month));
  if (!cycles.length) return withTruncationFlag<RevolvingCycleRow[]>([], false);

  const storedByMonth = new Map<string, StoredRevolvingCycle & { month: string }>();
  ...
  const derived: RevolvingCycleRow[] = [];
  let carried = 0;
  ...
  for (let month = start; month.localeCompare(end) <= 0; month = addMonths(month, 1)) {
    ...
    const action: PaymentAction = isValidPaymentAction(stored?.paymentAction) ? (stored?.paymentAction ?? null) : null;
 ...
 * @returns {boolean}
 */
export function isRevolvingChainTruncated(derivedCycles: unknown[] & { truncated?: boolean } = []): boolean {
 ...
 */
export function summariseRevolving(derivedCycles: RevolvingCycleRow[] = [], options: { asOfMonth?: string } = {}): RevolvingSummary {
...
export function resolvePlanStatus(plan: PlanLike = {}): PlanStatusValue {
...
export function summarisePlan(plan: PlanLike = {}): PlanLike & {
  installmentsPaid: number;
  installmentsRemaining: number;
  progressPercent: number;
  nextUnpaid: { no?: number; dueMonth?: string; payment?: number | string } | null;
 ...
 */
export function resolveCardNextDueDate(card: CardLike = {}, plans: PlanLike[] = [], derivedCycles: RevolvingCycleRow[] = []): string | null {
...
export function summariseCard(
  card: CardLike = {},
  plans: PlanLike[] = [],
  cycles: StoredRevolvingCycle[] = [],
  options: { asOfMonth?: string } = {}
): CardLike & {
  annualRate: number;
  minPaymentPercent: number;
  remainingPayable: number;
  remainingPrincipal: number;
  installmentPayable: number;
  installmentPrincipal: number;
  revolvingTotalDue: number;
  revolvingMinDue: number;
  revolvingDue: number;
  revolvingOutstanding: number;
  revolvingCarryForward: number;
  revolvingAction: PaymentAction;
...
export function summariseTotals(summarisedCards: Array<Record<string, unknown>> = []): {
  remainingPayable: number;
  remainingPrincipal: number;
  creditLimit: number;
  availableCredit: number;
  ...
  const totals = summarisedCards.reduce<{
    remainingPayable: number;
    remainingPrincipal: number;
    creditLimit: number;
    availableCredit: number;
...
type CardInputLike = {
  ...
  minPaymentPercent?: number | string | null;
  statementDay?: number | string | null;
 ...
 */
export function validateCardInput(input: CardInputLike = {}): {
  valid: boolean;
  ...
  value: {
    ...
    annualRate: number;
    minPaymentPercent: number;
    statementDay: number | typeof END_OF_MONTH_DUE_DAY;
    dueDay: number | typeof END_OF_MONTH_DUE_DAY;
  };
} {
  ...
  const color = typeof input.color === 'string' && CARD_COLORS.includes(input.color) ? input.color : CARD_COLORS[0];
...
type RevolvingCycleInputLike = {
  cardId?: unknown;
  month?: unknown;
  newSpend?: number | string | null;
};
 ...
 */
export function validateRevolvingCycleInput(input: RevolvingCycleInputLike = {}): {
  valid: boolean;
  errors: Record<string, string>;
  value: { cardId: string; month: string; newSpend: number };
} {
  const errors: Record<string, string> = {};
...
type PlanInputLike = {
  ...
  manualFeePerMonth?: number | string | null;
  annualRate?: number | string;
  calcMethod?: unknown;
};
 ...
 * @returns {{valid: boolean, errors: object, value: object}}
 */
export function validatePlanInput(input: PlanInputLike = {}): {
  valid: boolean;
  errors: Record<string, string>;
  value: {
    cardId: string;
  ...
  const interestMode = typeof input.interestMode === 'string' && INTEREST_MODES.some(mode => mode === input.interestMode)
    ? input.interestMode
    : 'manual';
  ...
  if (interestMode === 'manual') {
    ...
    if (typeof input.calcMethod === 'string' && CALC_METHODS.some(method => method === input.calcMethod)) {
      calcMethod = input.calcMethod;
    } else {
      errors.calcMethod = 'กรุณาเลือกวิธีคำนวณ';
    }
  }
